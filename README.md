...
| 항목 | 내용 |
| --- | --- |
| 서비스 목표 | 외국인 여행자가 한국 여행 중 필요한 행사, 교통, 음식, 편의시설, 일정을 하나의 앱에서 확인할 수 있도록 지원 |
| 핵심 흐름 | 홈 → 교통 챗봇 → 추천 경로 비교 → 경로 상세 저장 → 음식 알레르기 필터 → 행사 탐색 |
| 차별점 | Gemini 자연어 분석과 TMAP 실시간 교통 데이터를 연결해 실제 이동 가능한 경로를 추천 |
| 사용자 가치 | 낯선 지역에서도 이동, 식사, 행사 탐색, 일정 저장을 빠르게 처리할 수 있는 모바일 여행 도우미 |
